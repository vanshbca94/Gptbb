...
class LovableOrchestrator:
    ...
    def generate(self, request: str, status_cb=None, existing_files: dict = None) -> dict:
        ...
        u(f"✅ Domain: *{analysis.get('domain', '?')}* | Style: {analysis.get('visual_style', '?')} | "
          f"Type: {analysis.get('detected_type', '?')} | Features: {feats_count} critical + {adv_count} advanced | Unlimited complexity")
        ...
        u(f"✅ {len(arch.get('files', []))} file(s){ai_tag} | {len(arch.get('cdns', []))} CDN(s) | "
