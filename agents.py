...
class PromptAnalyzerAgent:
    ...
    def run(self, request: str) -> dict:
        ...
        prompt = f"""You are the world's best software requirements analyst and UX strategist. Deeply analyze this request and extract EVERYTHING needed to build an exceptional product.

USER REQUEST: "{request}"

Detected signals: {json.dumps({k: v for k, v in traits.items() if v})}

Output ONLY this JSON (no other text, be very specific):
```json
{{
  "original": "{request.replace('"', "'")}",
  "enhanced_request": "Detailed 2-sentence description of what to build, with specific features, target users, and visual style",
  "detected_type": "management|landing|webapp|dashboard|game|3d-scene|api|ecommerce|social|tool|portfolio",
  "domain": "specific domain (e.g., hospital patient management, neon snake game, SaaS AI writing tool)",
  "brand_name": "catchy professional name if not given",
  ...
  "ui_complexity": "simple|standard|complex|enterprise",
  "target_user": "who uses this app",
  "key_entities": ["PrimaryEntity", "SecondaryEntity"],
  "critical_features": [
    "Very specific feature 1 with implementation detail",
    "Very specific feature 2",
    "Very specific feature 3",
    "Very specific feature 4",
    "Very specific feature 5",
    "Very specific feature 6",
    "Very specific feature 7",
    "Very specific feature 8"
  ],
  "ui_sections": ["Section1", "Section2", "Section3", "Section4"],
  "data_fields": ["field1", "field2", "field3"],
  "animations": ["animation type 1", "animation type 2"],
  "must_include_libs": ["Tailwind CSS", "Font Awesome 6"],
...
```"""
        ...
        analysis.setdefault("ui_complexity", "complex")
    ...
    def _smart_fallback(self, req: str, traits: dict) -> dict:
        ...
        features_map = {
            "hospital": ["Patient registration with photo & medical ID", "Doctor scheduling & availability calendar", "Appointment booking system with reminders", "Medical records & diagnosis history", "Prescription & medication tracking", "Lab results management & reports", "Billing & insurance claim processing", "Real-time bed availability dashboard"],
            "student": ["Student enrollment & profile management", "Course registration & timetable builder", "Grade tracking with GPA calculator", "Attendance marking & analytics", "Teacher assignment per subject", "Parent portal with notifications", "Academic performance charts", "Exam scheduling system"],
            "hr": ["Employee onboarding & profile directory", "Department & team management", "Leave request & approval workflow", "Payroll calculation & slip generation", "Performance review system (360°)", "Recruitment pipeline tracker", "Training & certification records", "HR analytics & headcount reports"],
            "inventory": ["Product catalog with SKU & barcode", "Real-time stock level tracking", "Low stock alerts & reorder automation", "Supplier management & purchase orders", "Sales & dispatch management", "Inventory valuation reports", "Batch tracking & expiry alerts", "Warehouse location mapping"],
            "game": ["Smooth 60fps game loop with delta time", "Player with full keyboard & touch controls", "Collision detection system", "Score system with localStorage high score", "Lives & health system with visual feedback", "Level progression with difficulty scaling", "Web Audio API sound effects", "Particle effects on events"],
            "ecommerce": ["Product grid with filters, search, sort", "Shopping cart with quantity controls", "Wishlist with heart toggle", "Product quick-view modal", "Checkout flow with order summary", "Category navigation & breadcrumbs", "Product ratings & reviews display", "Responsive design with mobile optimization"],
            "landing": ["Full-screen hero with animated headline & CTA", "Sticky navbar with smooth scroll", "Features grid with icons & descriptions", "Pricing tiers (Free/Pro/Enterprise) with toggle", "Testimonials with avatars & quotes", "FAQ accordion with animations", "Newsletter signup form", "Footer with links & social icons"],
            "dashboard": ["KPI stat cards with trend indicators", "Interactive Chart.js line/bar/doughnut charts", "Data table with sort, filter, pagination", "Date range picker for filtering", "Export to CSV functionality", "Notifications panel", "Dark/light mode toggle", "Responsive sidebar navigation"],
            "crm": ["Contact management with pipeline stages", "Deal tracking & revenue forecasting", "Activity timeline per contact", "Email & call log history", "Task management & reminders", "Sales analytics & conversion reports", "Lead scoring system", "Team assignment & collaboration"],
            "3d": ["Interactive Three.js scene with orbit controls", "Physics-based animations", "Dynamic lighting system (ambient, directional, point)", "Particle system with 2000+ particles", "Post-processing effects", "Touch & mouse controls", "Performance-optimized render loop", "Loading screen with progress bar"],
        }
        ...
        return {
            ...
            "ui_sections": ["Dashboard", "Records", "Reports", "Settings"],
...
class CoderAgent:
    ...
    def _webapp_prompt(self, req, name, features, color, cdn_tags, views, analysis) -> str:
        ...
        prompt = f"""Write a COMPLETE, PRODUCTION-READY, VISUALLY STUNNING single-file HTML web app. MINIMUM 400 LINES. ZERO placeholders. ZERO TODOs.
...
REQUIRED FEATURES — implement ALL completely:
{feat_str}

MANDATORY TECHNICAL REQUIREMENTS:
1. <!DOCTYPE html> + complete <head> with viewport meta, title, all CDN links
2. Tailwind CSS v3 — use extensively: gradients, glass effects, shadows, animations
3. Full dark mode using 'dark:' classes, toggled with a button, saved to localStorage
4. Alpine.js x-data for ALL reactive state — no vanilla JS state management
5. CSS custom properties: --brand, --bg, --surface, --border colors
6. HERO or main section: gradient background, glassmorphism cards, floating elements
7. Sticky navbar: logo/title left, nav links center, dark toggle + CTA right, mobile hamburger
8. Multi-section layout: each section with smooth scroll-to, minimum 5 distinct sections
9. ALL data stored in localStorage with JSON.parse/stringify
10. Loading animation on page start (spinner or skeleton screens)
11. Toast notification system (top-right corner, auto-dismiss 3s)
12. Modal/dialog system with backdrop blur
13. Form with client-side validation (red border + error messages)
14. Smooth CSS transitions on ALL interactive elements (hover, focus, click)
15. Font Awesome 6 icons on ALL buttons, nav items, cards
16. AOS scroll animations on section entries (if AOS loaded)
17. Chart.js charts if this is data-heavy (if Chart.js loaded)
18. Mobile responsive: hamburger menu, stacked cards, touch-friendly buttons
19. Keyboard shortcuts: Escape=close modals, Enter=submit forms, Ctrl+K=search
20. Empty states with helpful icons and CTAs
21. Pagination if lists exist (10 per page)
22. Search/filter functionality with live results
23. Absolutely NO generic "Web App" title — use the actual app name: {name}
24. Absolutely NO bullet list of "features included" — show the ACTUAL working app

VISUAL QUALITY STANDARDS:
- Background: dark gradient (gray-950 to gray-900) or deep color
- Cards: bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl shadow-xl
- Buttons: gradient backgrounds, hover effects, active scale transform
- Text: white/gray-100 for primary, gray-400 for secondary
- Accent color {accent} for interactive elements, badges, charts

CDNs (already loaded in <head>, do NOT add again):
{cdn_tags}

Output ONLY the complete HTML in ```html block. No explanations."""
...
"""
    ...
    def _app_js(self, fspec, plan, arch) -> str:
        ...
        prompt = f"""Write COMPLETE, PRODUCTION-READY app.js for a "{req}" web app. MINIMUM 150 LINES. All functions fully implemented.
...
10. formatDate(dateStr): returns readable date string
11. formatCurrency(n): returns '$X,XXX.XX' string
12. confirmDelete(id, deleteFn, refreshFn): Opens confirm modal, on confirm calls deleteFn(id), refreshFn()
13. safeChart(id, config): Destroys existing chart on canvas, creates new Chart, stores in _charts[id]
14. printSection(): window.print()

DOMContentLoaded:
  - Load theme from localStorage, apply 'dark' class if needed
  - Call loadAllData() from data.js
  - Wire ALL .nav-link clicks: e.preventDefault(), get data-view, call showView(v)
  - Wire #modal-overlay click: if e.target === mo, closeModal()
  - Wire keyboard: Escape → closeModal(), Ctrl+K → focus #global-search
  - Wire sidebar toggle button if exists
  - Call showView('{first_view}')

All functions MUST be in global scope (no ES modules, no const/let for function declarations).
Output ONLY the complete ```javascript."""

        resp = call_ai(prompt, timeout=140)
        code = extract_code(resp, "javascript")
        if not is_quality_code(code, "javascript"):
            resp2 = call_ai(prompt, model=FALLBACK, timeout=110)
    ...
    def _data_js(self, fspec, plan, arch) -> str:
        ...
        domain_hints = {
            ...
            "hotel": "Rooms: id, roomNo, type(single/double/suite/deluxe), floor, price, status(available/occupied/maintenance/reserved), amenities[]. Bookings: id, guestName, email, phone, roomNo, checkIn, checkOut, status(confirmed/checked-in/checked-out/cancelled), totalAmount, payment.",
            "library": "Books: id, isbn, title, author, category, year, publisher, copies, available, status(available/borrowed/reserved). Members: id, memberId, name, email, phone, borrowedBooks[], joinDate, status(active/suspended). Borrowings: id, bookId, memberId, borrowDate, dueDate, returnDate, status.",
        ...
        prompt = f"""Write COMPLETE, PRODUCTION-READY data.js for a "{req}" web app. MINIMUM 120 LINES. All functions fully implemented.
...
1. SAMPLE DATA — minimum 15+ realistic records:
...
5. ANALYTICS:
   function getStats() — returns: total, active, inactive, recent (last 30 days),
     monthly (12-element array for each month), categoryBreakdown (object),
     plus 2-3 domain-specific metrics (e.g., avgGpa, totalRevenue, admissionRate)

6. UTILS:
   function generateId(prefix) — prefix + timestamp + random (e.g., 'S' + Date.now().toString(36).slice(-4).toUpperCase())
   function formatDate(iso) — 'Jan 15, 2024' format
   function getStatusColor(status) — returns badge CSS class name

All in global scope. No ES modules.
Output ONLY complete ```javascript."""
    ...
    def _ui_js(self, fspec, plan, arch) -> str:
        ...
        prompt = f"""Write COMPLETE, PRODUCTION-READY ui.js for a "{req}" web app. MINIMUM 200 LINES. All render functions fully implemented with real HTML.
...
class IterationAgent:
    ...
    def run(self, modification: str, existing_files: dict, plan: dict) -> dict:
        ...
        # Find best file to modify
        main_file = None
        for candidate in ["index.html", "app.py", "scene.js"]:
            if candidate in existing_files:
                main_file = candidate
                break
        if not main_file and existing_files:
...
Current {ext} file ({len(current_code)} chars):
...
5. If fixing a bug: fix ONLY that bug, touch nothing else
6. If changing colors/theme: update consistently throughout
7. Result must be production-ready — no placeholders
...
class PackagerAgent:
    ...
    def _readme(self, plan, arch, files) -> str:
        ...
        return f"# {name}\n\n{plan.get('description', '')}\n\n## Features\n{feats}\n\n## Stack\n{tech}\n\n## Files\n{flist}\n\n## Quick Start\n{setup}\n\n---\n*LovableBot — 8-Agent AI Platform*\n"
    ...
    def _summary(self, plan, files, dbg, total) -> str:
        ...
        flist = "\n".join(f"  📄 `{p}` — {len(c):,} chars" for p, c in sorted(files.items()))
        fixes = ("\n\n🔬 *Auto-fixes:* " + ", ".join(dbg[:3])) if dbg else ""
        return (
            f"✅ *{name}* is ready!\n\n"
...
class LovableOrchestrator:
    ...
    def generate(self, request: str, status_cb=None, existing_files: dict = None) -> dict:
        ...
        u(f"{self.analyzer.emoji} **Agent 0/8 — Prompt Analyzer**: Deep analyzing your request...")
        analysis = self.analyzer.run(request)
        u(f"✅ Domain: *{analysis.get('domain', '?')}* | Style: {analysis.get('visual_style', '?')} | "
          f"Type: {analysis.get('detected_type', '?')} | Color: {analysis.get('primary_color', '?')}")

        u(f"\n{self.planner.emoji} **Agent 1/8 — Planner**: Creating strategic project plan...")
        plan = self.planner.run(request, analysis)
        u(f"✅ *{plan.get('project_name')}* | {len(plan.get('features', []))} features | "
          f"Type: {plan.get('project_type')} | Theme: {plan.get('color_theme')}")

        u(f"\n{self.architect.emoji} **Agent 2/8 — Architect**: Designing file architecture...")
        arch = self.architect.run(plan)
        u(f"✅ {len(arch.get('files', []))} file(s) | {len(arch.get('cdns', []))} CDN(s) | "
